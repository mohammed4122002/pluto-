...
from app.services.directory import (
    find_doctor_across_branches,
    find_doctors,
    list_active_branches,
    list_services,
    resolve_branch_id_by_name,
)
...
BASE_INSTRUCTIONS = (
    ...
    "- **find_doctors مرتبطة بالفرع الحالي بس** — رجعت فاضية أو من غير الاسم يلي سأل عنه المريض معناها "
    "مش موجود **بهذا الفرع تحديداً**، مش إنه مش موجود إطلاقاً. لو المريض سأل عن **طبيب باسمه** بالذات "
    "ولقيتيه مش موجود بفرعه الحالي، ممنوع منعاً باتاً تقولي 'ما في طبيب بهذا الاسم عنا' أو 'مش موجود "
    "بأي فرع' أو أي صياغة توحي إنه مش موجود بالعيادة كلها — لازم تستدعي find_doctor_any_branch أول "
    "قبل أي جواب نهائي. هذا حصل غلط فعلياً وبنفس المحادثة: قيل للمريض إن 'د. ليان' مش موجودة عندنا "
    "بأي فرع، بينما كانت موجودة وقابلة للحجز بفرع تاني لسا ما تفحّص. لو find_doctor_any_branch رجعت "
    "فاضية هي كمان، عندها بس اقولي إنه مش موجود بأي فرع من فروعنا.\n"
    ...
    "- لما المريض يسأل عن **الأوقات المتاحة**، اذكريله **كل الأوقات** اللي رجعت من الأداة، مش عيّنة منها — موظفة الاستقبال ما بتخبّي مواعيد فاضية عن المريض. رتّبيها بكارت: عنوان بولد لكل تاريخ مع 📅، وتحته كل وقت بسطر لحاله ('• 🕐 09:00 ص'). لو الأوقات كثيرة جداً (أكتر من يومين-تلاتة)، اعرضي أقرب يومين-تلاتة وقولي إن في مواعيد أيام تانية واسأليه أي يوم بفضّل.\n"
    "- **عنوان كل تاريخ لازم يكون بالضبط قيمة حقل day الراجعة من find_available_slots لهذا الموعد "
    "('اليوم'، 'بكرة'، أو 'الخميس 2026-09-17' مثلاً) — انسخيها حرفياً، ما تحسبي اسم اليوم بنفسك ولا "
    "تعيدي صياغتها. هذا حصل غلط فعلياً: الأداة رجعت day='بكرة' لموعد يوم الخميس 17، بس الردّ كتب "
    "'الأربعاء 17 سبتمبر' (حسبت اسم اليوم بنفسها غلط بدل ما تنسخ القيمة الراجعة). حساب اسم اليوم من "
    "تاريخ ممنوع نهائياً — دايماً انسخي day كما هي.\n"
    ...
    "- **أول مرة** بالمحادثة يذكر المريض جلدية/تجميل/أسنان (حجز أو وصف مشكلة بشرة/شعر/أسنان، مثل 'عندي "
    "حبوب' أو 'ضرسي بيوجعني') — اذكريله بجملة قصيرة إنه بإمكانه يرسل صورة حالته لتشخيص مبدئي وأنسب "
    "خدمة، وطمنيه إنها آمنة. مرة وحدة بالمحادثة بس، ولا تفرضيها لو أصلاً بعت صورة أو حدد الخدمة.\n"
    "\n"
    ...
    "    **استثناء لجلدية/تجميل/أسنان بس**: سؤال عن علاج/دواء/تشخيص لحالة بشرة/شعر/أسنان بدون صورة "
    "مرفقة، ممنوع تصعّدي — اقترحي يرسل صورة لتشخيص أدق وأنسب خدمة. لو رفض، جاوبيه بخدمة مناسبة من "
    "list_services بس (مش اسم دواء أو تشخيص)، ولسا ما تصعّدي. باقي التخصصات تبقى تصعّد عادي.\n"
...
TOOLS = [
    ...
    },
    {
        "type": "function",
        "function": {
            "name": "find_doctor_any_branch",
            "description": (
                "لما المريض يسأل عن **طبيب باسمه** وfind_doctors رجعت من غيره بهذا الفرع، استدعي هذه "
                "الأداة لتتأكدي هل هو موجود بفرع تاني قبل ما تقولي للمريض إنه مش موجود عندكم إطلاقاً. "
                "ممنوع منعاً باتاً تقولي 'ما في طبيب بهذا الاسم عنا بأي فرع' أو ما يشابهها بدون ما "
                "تستدعي هذه الأداة فعلاً وترجع فاضية — هذا حصل غلط فعلياً: قيل للمريض إن 'د. ليان' مش "
                "موجودة عندنا بأي فرع، بينما كانت فعلاً موجودة وقابلة للحجز بفرع تاني لسا ما تفحّص. لو "
                "رجعت النتيجة موجودة، اذكري اسمها وفرعها وتخصصها بوضوح واسألي المريض إذا حابب تحوّليه "
                "لهذا الفرع (استدعي select_branch لو وافق) قبل أي حجز فيه."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "doctor_name": {
                        "type": "string",
                        "description": "اسم الطبيب أو جزء منه، بالضبط متل ما ذكره المريض.",
                    }
                },
...
def _execute_tool(db: Client, ctx: dict, name: str, args: dict) -> dict:
    try:
        ...
        if name == "find_doctor_any_branch":
            return {"doctors": find_doctor_across_branches(db, ctx["branch_id"], args.get("doctor_name") or "")}
        ...
        if name == "book_appointment":
            ...
            result = {
                ...
                # Pre-computed so the confirmation card's "📅 يوم" line never
                # asks the model to work out a weekday itself -- see
                # book_by_doctor_and_time's own comment for the live incident.
                "day": appointment.get("day"),
                "scheduled_at_clinic_local_time": appointment.get("scheduled_at_clinic_local_time"),
