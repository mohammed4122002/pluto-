...
def require_service_token(request: Request, x_service_token: str | None = Header(default=None)) -> None:
    """Gates the machine-to-machine endpoints n8n calls directly (no staff
    login involved). See SERVICE_AUTH_MODE in .env: "permissive" logs a
    warning on a missing/wrong token but still lets the request through —
    used during the rollout window before every n8n workflow sends the
    header; "strict" actually rejects with 401."""
