...
The ai-services chat router referenced `payload` -- a parameter of the route
function, not of the helper that used it. Python only raises on the line being
reached, the generic `except Exception` around every tool call turned it into
"صار خطأ تقني بسيط", and it reached production, where it broke saving a
patient's contact details and therefore every first-time booking.
